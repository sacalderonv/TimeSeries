...
<b>ggplot2</b>. Vamos empezar con la serie USgas.
...
UKgrid_tbl%>%mutate(diff_ND=ND-lag(ND))%>%plot_time_series(TIMESTAMP,diff_ND,.smooth=F)
...
```
Tarea: Por favor hacer los análisis descritos anteriormente,es decir los gráficos de barras de las medias por horas, días de la semana, y meses para detectar la estacionalidad tanto para serie diferenciada, como para la serie cuando se estima la tendencia y luego se elimina(Use fitro de promedios móviles o STL).
...
#write_csv(data.frame(soi=as.matrix(soi),date=as.Date(as.yearmon(time(soi)))),
file = "soi.csv")

#write_csv(data.frame(rec=as.matrix(rec),date=as.Date(as.yearmon(time(rec)))),
file = "rec.csv") #\`\`\`
