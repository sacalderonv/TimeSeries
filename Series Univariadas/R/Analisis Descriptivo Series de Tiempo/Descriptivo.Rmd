...
##"/Users/macbook/Library/r-miniconda/bin/python"
...
$$x_t=\frac{x_t-1}{x_{t-12}^2+1}+\epsilon_t$$ y a trabajar con la serie de linces Candienses del paquete astsa lynx.
...
Otra forma de introducir la estacionalidad de forma estocástica es permitir que sea cambiante en el tiempo sin ningún valor fijo medio. En este caso la estacionalidad puede proponerse que siga un porceso no estacionario por ejemplo que siga una caminata aleatoria:
$$S_t^{(s)}=S_{t-s}^{(s)}+\nu_t$$
...
Al igual como se hizo con la tendencia, hay al menos dos forma de eliminar la componente estacional:
* Estimando la componente estacional y luego proceder quitarla.
* Usando diferencias estacionales $(1-B^s)^D$, donde $s$ es el periodo del ciclo estacional.
