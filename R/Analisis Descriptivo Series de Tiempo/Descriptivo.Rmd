...
#Ygrid = TRUE)
```
Note que aquí podemos ver varias características: Estacionalidad(no es tan evidente), tres periodos de ciclos, el primero de 1990 a 2000,el segundo de 2000 a 2007, y el tercero de 2007 a 2019. No parece tener una hetocedasticidad marginal.
...
Veamos ahora con una serie de tiempo real. Usaremos la serie SOi y Recruitment para ilustrar su uso. Recordemos que ambas series son mensuales, así que las frecuencias que serán dibujadas en el eje x vedrán en múltiplos de $\frac{1}{12}$. Usaremos ahora la función <b> mvspec</b> del paquete <b>astsa</b>.
...
sprintf("El valor de la frecuencia donde se máximiza el periodograma para SOI es: %s",rec.per$freq[ubicacionrec])
...
sprintf("El valor de la frecuencia donde se alcanza el segundo máximo para el periodograma para SOI es: %s",rec.per$freq[ubica_segundo_rec])
...
donde si$a_j=a_{-j}\ge0$ y $\sum_{j=-k}^{k}a_j=1$ se conocce como el el promedio móvil simétrico de los datos.
...
son los pesos y $K(\cdot)$ es una funció kernel. Este estimador es llamado el Estimador de Nadaraya-Watson. Usaremos la función ksmooth
