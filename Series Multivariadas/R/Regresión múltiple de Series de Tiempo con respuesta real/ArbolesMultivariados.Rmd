...
La poda del árbol puede incluir los datos de validación para  elegir el modelo.

```{r poda}
cv.tree.fit= cv.tree(tree.fit)
plot(cv.tree.fit$size,cv.tree.fit$dev,type="b")##Parece que el tamaño del árbol es 2
prune.tree.fit= prune.tree(tree.fit,best=2)
plot(prune.tree.fit)   ####Gráfica de árbol con la poda
text(prune.tree.fit,pretty=1)
```
